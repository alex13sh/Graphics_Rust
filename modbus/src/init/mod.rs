...
pub fn make_owen_analog_2(ip_addres: &str, id: u8) -> Device {
    ...
    let make_sensor_davl = |pin, name: &str, value_error: (f32, f32)|
        make_values(pin, name, value_error.into(), 
            ...
            ValueSize::UINT16
//             ValueSize::UInt16Map(|v| v as f32 * 1.0/ 100.0)
//             ValueSize::UInt16Map(|v| v as f32 * 1.0)
        );
...
pub fn make_o_digit(ip_address: String) -> Device {
    ...
    Device {
        ...
        values: Some(vec![
            ...
            make_klapan(12, "Клапан помольной камеры" ), // "Насос"
            make_klapan(11, "Клапан напуска" ),
            make_klapan(13, "Клапан верхнего контейнера"), //"Клапан подачи материала в камеру" ),
